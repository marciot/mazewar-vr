/* This file implements the Alto Ethernet interface. The implementation here is
 * abridged and differs from the implementation used in Contralto. Contralto
 * simulates the sixteen word FIFO queue and stops and wakes the Ethernet task
 * at the correct intervals at which real 3mbit Ethernet bytes would arrive.
 *
 * This implementation does not simulate the FIFO queue or timing; instead words
 * are read and written to byte arrays and the Ethernet microcode task is coerced
 * into writing or reading whole Ethernet frames in one go.
 *
 * This implementation appears to work, and is simpler than the original, but
 * since it causes the Ethernet task to run longer than it would in a real Alto,
 * it is possible it may cause unforeseen issues.
 *
 * A complete implementation will be provided for comparison purposes as
 * ethernet_controller.js
 *
 * Authors:
 *   - Seth J. Morabito
 *   - Marcio Teixeira
